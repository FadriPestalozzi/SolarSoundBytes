...
def get_database_url():
    ...
    if not db_url:
        raise ValueError("DATABASE_URL not found in environment variables. Please check your .env file or Streamlit secrets.")

    # Handle SQLite paths: resolve relative paths to absolute paths
    if db_url.startswith('sqlite:///'):
        # Convert sqlite:///database/file.db to absolute path
        # This file is at: website/data_analysis/import_newsarticle_sent_analysis.py
...
def create_df_of_newsarticle_result():
    """
    Load news articles with sentiment analysis from PostgreSQL database.
